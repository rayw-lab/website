...
| 体积 | **146,464 B（≈143 KB）** ≤ 10MB spike 合同；SHA-256 `d181147311f2af432706edd2c695de503f8d7281e4a762fa5a8d9072160988aa` |
| 几何 | 2,586 tris（合同 ≤100k）· 13 具名材质 primitive = 13 draw call |
...
| 内容 | concept-garage 沿街 hero 楼实模（3 间暖光展厅橱窗+概念展车 / 门厅无柱悬挑雨棚 / 双卷帘门+服务窗 / 中段逐窗幕墙 / 设备顶带 / 屋顶 HVAC+桅杆蓝信标）+ 南前场道具簇（室外展车台、配置器 kiosk、横幅旗杆×2、备件箱堆、轮胎堆、服务推车、地面导视光条） |

## 坐标与合同（消费端 `src/lab/world/city/HeroBlenderMesh.ts`）

- 原点 = 楼体足迹中心地面点；运行时平移到 buildings JSON `position` (140,−44) 并按
  `rotationY` 旋转。视觉包络 = footprint w60×d36×h18 同笼（物理碰撞体沿用
  ThemeTowers footprint cuboid，不随实模改动；前场道具碰撞体在
  `HeroBlenderMesh.PROP_COLLIDERS['concept-garage']` 注册 9 件）。
- BuildingSigns 南立面灯箱挂点（by=−(18+0.35)、挂高 min(25,max(9,0.34h))=9）已在
  模型侧留「招牌避让背板区」bx∈[−10.2,10.2] z∈[7.0,11.0]（FacadeDark 背板 0.05 凸，
  竖梃/层间梁截断让位）。
- 楼顶全息板走廊 bx∈[−10.5,10.5] × by∈[−1.8,1.8] 已让空（HVAC/桥架/桅杆全部偏轴，
  信标立前西角）。
...
- 色纪律（rubric A3）：窗格暖白/青/暗三族 atlas；楼宇身份色 #3b82f6 只进 LED
  檐口线/横幅/信标；工业警示件保持通用橙黑（不随楼染色）；emissive 全部 ≤1 阈下，
  唯一辉光锚 = 屋顶信标 + 卷帘门警灯（`BeaconBlue` strength 2.2 > bloom threshold 1）。

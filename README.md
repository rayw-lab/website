...
个人网站工程:Astro + TypeScript + MDX,部署目标为 GitHub Pages(项目页路径 `/mywebsite`)。总体方案见 `docs/website-plan/master-plan.md`,两条 Demo 路线的技术调研见 `docs/research/`。
...
```bash
git clone https://github.com/rayw-lab/mywebsite.git
cd mywebsite
git checkout cursor/full-site-delivery-1d6f
pnpm install
pnpm dev
# TTS Demo: http://localhost:4321/mywebsite/lab/tts-cockpit
# 3D Demo:  http://localhost:4321/mywebsite/lab/car-configurator
```
...
注意:`astro.config.mjs` 配置了 `base: '/mywebsite'`,因此所有页面都在 `/mywebsite/` 前缀之下;绑定自定义域名后需修改 `site` 并删除 `base`。
...
- `main` — 基线分支,仅含初始 README,等待审阅后合入。
- `cursor/full-site-delivery-1d6f` — **本交付分支**,合并了以下全部工作分支(保留完整历史):

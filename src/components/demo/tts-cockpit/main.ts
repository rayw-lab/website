/**
 * LAB RA-01 · 多语言 TTS × 座舱可视化 —— 交互岛（零依赖 vanilla TS）。
 *
...
const $s = (id: string): SVGGraphicsElement =>
  document.getElementById(id) as unknown as SVGGraphicsElement;
...
function applyScene(tMs: number) {
  switch (sceneId) {
    ...
    case 'tour': {
      ...
      const nodes = Array.from(document.querySelectorAll<SVGGElement>('.tour-node'));
...
function init() {
  initGaugeTicks();
  document.querySelectorAll<HTMLButtonElement>('[data-locale]').forEach((b) => {
    b.addEventListener('click', () => selectLocale(b.dataset.locale!));
  });
  document.querySelectorAll<HTMLButtonElement>('[data-scene]').forEach((b) => {

...
void main() {
  ...
  // 🔴 渐近软肩（W9）：几十滴叠在同一格时吸收量线性累加，exp(-a) 早已贴地，
  // 团块内部全是同一个底光值 —— 「死黑多边形」（agy W9 P0-2 属实）。
  // 把吸收量压进 k·(1-exp(-a/k))：低密度段几乎恒等（单滴不变），高密度段趋近 k=2.9
  // （exp(-2.9)≈5.5%，恰在底光之上），叠加层次因此保留而不是被夹持抹平。
  absorbed = 2.9 * (1.0 - exp(-absorbed / 2.9));

...
export class InkEngine {
  ...
  drop(
    ...
    // 冲量上限：防调用方传入极端 vigor 把速度场推爆。
    // 🔴 阈值依据（二分实测，2026-09-04）：上限 48 / 56 / 64 三档出图几乎完全相同
    // （非纸色 98.97% / 98.98% / 98.97%），说明 kick 在这个区间**不是敏感参数**。
    // 早先「kick≈66 会把颜料吹散」的结论**已被推翻** —— 那次同时把湿斑数从 5 改到 10，
    // 真凶是湿斑数量翻倍，不是 kick 值。取 56（实测安全 64 的约 87%）留余量。
    const kick = Math.min(56, 34 * (radius / 0.03) * vigor);

...
export class InkEngine {
  ...
  drop(
    ...
    // 🔴 绝对上限：W1b 实测 kick≈66 会把颜料整个吹散、画面全白，而且引擎不报错、
    // ready 正常、console 干净 —— 最难查的一类。真实数据的 tools/turns 比远高于
    // 伪随机，会把调用方的 vigor 顶到上限，所以上限必须由**引擎**兜住，
    // 不能指望每个调用方自觉。48 是实测安全区（r=0.058 线性缩放时 47 稳定出图）。
    const kick = Math.min(48, 34 * (radius / 0.03) * vigor);

...
// 高楼（≥55m）双阶 setback + 楼顶天线信标；矮楼/横向体量单体块 + 招牌带。
...
export class ThemeTowers {
  ...
  private addTower(building: Building): WorldObject {
    ...
    if (h >= 55) {
      ...
      signageCenterY = h / 2 - 2.2;

      // 招牌带（全息招牌文字归 CC-E4/E9，此处为霓虹占位带）
      addBox(
        w * shrink * 1.05,
        1.5,
        d * shrink * 1.05,
        signageCenterY,
        createNeonGlowMaterial(building.neonColor, { pulseSpeed: 0.9, phase: seed % 6, intensity: 2.2 }),
      );
    } else {
      addBox(w, h, d, 0, facade);
      signageCenterY = h / 2 - Math.min(1.6, h * 0.12);
      addBox(
        w * 1.04,
        Math.min(1.5, h * 0.1),
        d * 1.04,

...
/**
 * [CC-L2-a+] 城市水洼掩码单源（Grid 地坪与 Roads 路面共用同一世界坐标噪声场，
 * 图案跨网格无缝）：低频价噪声全城分布 + 首幕前景「英雄水洼」椭圆软区。
 * AL2-a §6-1 判词：A9 收益主要停在画面右缘（Grid 广场）——主体脚下 (0,0) 与
 * 斑马线带其实是 Roads 路面，全城噪声又不保证前景恰好积水。英雄区推导
 * （View.ts 城市档实拍机位）：机位 ≈(12.0, 5.2, 15.7)（θ25° 偏轴 + 右移 4.2m），
 * 视线压向出生点——帧内前景地面 = 路口东南象限 + 南进口斑马线带（z≈13.2-16）。
 * 椭圆中心 (3, 8)、半轴 (13, 11.5) 恰盖「主体脚下 → 斑马线 → 近机位路面」，
 * 北向路廊 (z<-4) 零覆盖（远景干湿对比保留）；区内积水度 = 0.6-1.0 噪声有机调制
 * （下限 0.6 保证主体脚下必湿，上沿随噪声起伏防「盖章椭圆」感）。
 */
export function cityPuddleMask(): FloatNode {
  const noise = valueNoise(positionWorld.xz.div(19).add(vec2(3.7, 8.1)) as Vec2Node);
  const ambient = smoothstep(0.3, 0.64, noise);

  const ex = positionWorld.x.sub(3).div(13);
  const ez = positionWorld.z.sub(8).div(11.5);
  const heroZone = smoothstep(1.05, 0.5, ex.mul(ex).add(ez.mul(ez)));
  const heroWet = heroZone.mul(smoothstep(0.02, 0.5, noise).mul(0.4).add(0.6));

  return ambient.max(heroWet) as FloatNode;
}

/** [CC-L2-a+] Q1 假反射 sheen 色（青/品红按低频噪声混色）——Grid/Roads 共用单源 */
export function citySheenColor() {
  return mix(
    vec3(0.06, 0.5, 0.44),
    vec3(0.62, 0.02, 0.14),
...
export class Grid {
  ...
  /** [CC-L2-a+] Q0 实时反射节点（Roads 路面湿反射层共享同一镜像渲染，零二次开销；
   *  applyQuality(0) 之后才非 null——city/index.ts 接线保证 Grid 先切档） */
  get reflectionNode(): ReturnType<typeof reflector> | null {
   ...
   *  [CC-L2-a+] 掩码抽为模块级 cityPuddleMask（含首幕前景英雄湿区），Roads 路面共用 */
  private puddleMask() {
    return cityPuddleMask();
  }
  ...
  applyQuality(level: QualityLevel): void {
    ...
    if (level === 0) {
      ...
      const puddle = this.puddleMask();
      this.material.emissiveNode = grid.add(citySheenColor().mul(puddle).mul(0.2));

...
## 附录 P.S.（06:22 UTC 交付前复核）

1. **重试壳已自然耗尽**：attempt3 于 06:20 成功出数自退（`PROBE_RESULT` 落盘 probe-plug.out，壳按「OK on attempt 3」退出），06:22 ps 实测 probe-loop/fps-probe/chrome **零谱系**，load 1min 1.75——**真空在望，run3 放行窗可提前至 ~06:25–06:30**。§2 根杀令对象已消亡，**缴械+断靶+冻结+重试壳禁令不豁免**（main 腿有前科，靶与弹药在盘即可再武装）。
2. **新读数入档并对撞 F6**：attempt3 `PROBE_RESULT {fps:0.699, designDeltaS:2.1, designRatePerWallS:0.0233, wallSecondsFor30DesignS:1288}`——与探针 #2 的 `designRate 1.0225` **直接矛盾**（同 URL 同脚本，0.7fps 下设计秒几乎不走 vs 0.4fps 下照墙钟累积）。T21「绝对读数跨窗不可比」告警再次坐实且升级：**「假 ✓ 机制」的定量口径本身待重测**，两读数一并挂 #129 证据链存疑区，A/B 议程 ✓✓ 后真空窗背靠背重做，此前禁止引用任一读数作结论。

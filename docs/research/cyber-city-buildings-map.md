...
3. **五大城区**：南口语言双子（多语种/TTS）、AI 中枢（Agent/端云/工作流/状态塔）、东城出行（智驾/车库）、东南作品区（Work/Insights）、个人区（About/Contact）——城区共享霓虹色族，迷你地图按区渲染。方位词已从街区 title 拿掉（ADR-4 换槽后归属不换）。
...
| `ai-core` | AI 中枢区 | 03 agent-nexus · 10 edge-cloud-hub · 11 workflow-foundry · 12 now-signal | 紫/绿/银 | 机器人「老家」，Master Agent 与端云算力聚集 |
...
| `civic` | 个人区 | 08 about-pavilion · 09 contact-beacon | 暖白 × 青柠 | 「找人」动线：了解王磊 → 取得联系 |
...
排布校验：按 ADR-4 决策 A，`about-pavilion`（个人档案馆）与 `now-signal`（状态塔）互换世界槽位，`about-pavilion` 迁至中轴正北槽位 (-44, -150) 作为开屏与进城第一站，`parkingBay` 随足迹重算保持距路缘安全距离；楼体近街边缘距路缘 ≥8m（留人行道 + 泊车位）；内环四角楼间峡谷口宽 64m，车速 ≤40km/h 可安全通过；天际高度差（18m 车库 ↔ 96m 中枢）保证剪影层轮廓不呆板。

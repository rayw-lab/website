...
| 决策状态 | **待王磊评审**——评审通过后 Fable5 按 `cyber-city-hero-fable5-tasks.md` 并行调研/细化 |
...
| # | 问题 | 选项 | 建议 |
|---|------|------|------|
| D1 | 首屏是否 **Full Entry**（`/` 即本场景） | A `/` 全屏科技城 / B 保留 HTML 首页 + 链到 `/city/` | **A**，但 DOM 壳保 SEO（技术方案 W1 壳页思路） |
| D2 | 机器人气质 | A 原创机甲 / B 更人形 / C 更工业风 | **A** + 法务：零 Transformers 商标元素 |
| D3 | 首屏城市精度 | A 纯程序化剪影 / B 1 套低模楼 GLB 实例化 | **A** 首版；B 进 Phase 1 |
| D4 | 变形后首屏停留 | A 仅展示车+提示 Phase 1 / B 首屏即可慢速绕台 | **A**（范围清晰） |
| D5 | PRD §11「赛博朋克主题」禁令 | 修订为「座舱科技城」白名单 | **建议修订**——领域强相关 |
| D6 | 门禁 | 接受 `/` Perf 分层考核 | 与 `full-entry-world-proposal-tech.md` §6 一致 |

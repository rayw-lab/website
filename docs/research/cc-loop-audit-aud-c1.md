...
- **W2' 跑中要点（20:34）**：#1 `CITY-EXP-01`（explore:285）**串行下亦挂**——与 §8「串行全绿=挤兑定谳」不符（R2 W2 该例 ✓18.1m、R3-R1 双 worker 下 ✓）：EXP-01 呈 SwiftShader 时序边缘型 flaky 特征（跨配置漂移），归因待终局 JSON 修正。

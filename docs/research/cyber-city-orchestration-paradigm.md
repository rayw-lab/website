...
### 1.3 事后顾问（董事会）：触发与权威（2026-08-28 指挥官授权，R1 [`cc-loop-board-advisor-r1.md`](cc-loop-board-advisor-r1.md) 首例）

- **触发条件（满足其一即可派，非触发不派）**：① 连续 ≥2 tick 无有效新增（卡点复读、合流拖欠、决定性趟被反复污染）；② 角色冲突（子代理互相污染/覆盖跑道或产出、纪律事件与实现内容纠缠、顾问链与执行链失衡）；③ 指挥官点名。
- **权威**：书面裁决 = 董事会决议，父代理与所有子代理必须执行；冲突时**优先于编排顾问 T* 与一切实现/审计/调研/脑暴/质量/性能/产品单**。裁决覆盖：角色停/续/重派、合流序与禁合项、跑道禁令与节流、口径终裁（如双源分歧定谳）。
- **纪律**：零 `src/` 业务代码；可写 `docs/research/` 裁决与看板增量；独立 worktree 一单一 PR，**董事会件优先审**；交付必含「角色冲突终裁表 + 立刻可执行合流序 + 父代理 Tick 立即执行清单（≤10 条）」；交付即 IDLE，不常驻。
- **model slug**：`claude-fable-5-thinking-xhigh`（降级遵循 AGENTS.md §3）。
...
| 14 | 静态「源码确定性复刻」人工推导出错（PRNG 序列/调用序/种子串任一偏差，rotY 实证最大差 10.80°（Cabinet）） | 复刻必须附**可执行重放脚本**（自包含内联源码函数 + 源码行号钉死），输出与文档数字 diff=0；先例 `docs/research/tools/streetprops-roty-replay.mjs` |

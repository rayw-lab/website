...
| 日期 | 2026-08-28（UTC）取证窗口 15:32Z–22:03Z（R4 终局轮 21:50Z 收 + PERF 对照双跑 22:03Z 收，全链闭账） |
...
3. **其余核验面全部过门**：DP-1 传送式逐条 PASS（§3）、DP-3 三重保险三层 PASS 且 **CITY-NAV-01/02/03 独占窗全绿 + VIS-01–04 全绿**（poster 恒等运行时终证）、文件域合规（§4）、禁 pointer-lock 零命中、LHCI 不降（CI artifact 回填，§5）。全量轮其余失败经**双向对照实验**（同 VM 无 #166 对照树同款失败、时长指纹 ±0.3m 一致）逐项定性：4 项本 VM 环境性 + 2 项仓库级规格恒红（CITY-PERF-01/02，先于 #166 存在、任何树恒败），全部与 #166 零因果（§2.1 对照表）；OBS 全链（01/01b/02–06）与 FB-05/06 等连坐项复跑后**全绿**。**#166 语义账 = 84 分母下与其有因果的失败仅 1 项（CITY-AUD-01）**。修复面极小（一处 CSS 落位 + 一条回归断言），**不推倒重来、不降门**，走定向补洞（§7 F1–F3）。
...
| R3（定向单跑清连坐） | FB-05 / FB-06 / OBS-04 / OBS-05 / OBS-06 | FB-05 ✓ · FB-06 ✓ · OBS-04 ✓ · OBS-05 ✓；OBS-06 ✘ 52ms = **过滤跑工件**（新 session 清空 test-results，其消费的 `session-dump-funnel.json` 需同 session 先跑 OBS-01 产出——报错原文自证），R4 连跑闭账 ✓ |
| R2 | `world-perf-chromium` → `city-perf-chromium` → `visual-chromium`（`--no-deps`） | WS-PERF-01 ✓；**VIS-01/02/03/04 全 ✓（poster/视觉基线逐字节合同运行时终证）**；CITY-PERF-01 ✘（`[data-ws-fps]` 30s 未现——执行时外来裁决复跑在途，R4 空窗重跑证实与竞争无关，见下） |
| R4（21:31Z 空窗终局轮，load<1 前后 `ps` 双查零外来） | CITY-PERF-01/02 + OBS-01\|01b\|06 同 session 连跑 | **OBS-01 ✓ 9.2m · OBS-01b ✓ 28.2s · OBS-06 ✓ 34ms**（同 session 工件链闭合，R3 的 OBS-06 定谳 = 跑法工件依赖、非产品缺陷）；CITY-PERF-01 ✘ 4.4m / PERF-02 ✘ 4.6m——**非竞争性**：①–④ 步（挂载/robot_idle/变形/car_ready/driving/速度>2/20s 脚本/rAF 采样出帧）全过，败点均为 H3 `[data-ws-fps]` element not found（30s），定性升级见下「规格恒红」 |
...
| CITY-PERF-01/02 | ✘ 4.4m / 4.6m（R4 空窗） | **✘ 4.4m / 4.6m**（main@`4a58789` 定向双跑，同款断言同时长指纹） | **规格恒红（先天，非环境性、非 #166）** |
...
**PERF-01/02 规格恒红定谳（三重互证）**：① 静态——断言的 `[data-ws-fps]` 只存在于 `/world-spike/` 页壳（`world-spike/index.astro` L68）；城市页 `/` 全史零该元素（`git log --all -S 'data-ws-fps' -- src/pages/index.astro` 零命中 + `dist/index.html` rg 零匹配），引擎侧只 `querySelector` 不创建且注释自书「缺席容忍」——`cyber-city-perf-test-plan.md` L90「HUD `[data-ws-fps]` 在 `/` 城市档已挂（两页共用装配段）」**前提失实**（共用的是引擎装配段，HUD DOM 归各页壳）；② 经验——#166 树与 main 对照树空窗双败、同断言同指纹（上表）；③ 独立旁证——AUD-C1 审计幽灵完账 §8.4（commit `a38f879`）同日独立得出同判。**修复归 CC-PERF 规格工单**（spec 改锚既有 `__worldSpike.fps()` 探针，或城市页补元素 = src 工单），**不入 #166 补洞范围**。

**裁决口径**：84 分母 = **77 例实测至少一绿 + 1 真回归（CITY-AUD-01，#166 账）+ 4 环境性（VM 账）+ 2 规格恒红（仓库账，先于 #166 存在）**——**与 #166 有因果的失败仅 1 项**。0/0/0 硬门：本 VM 因环境四项物理不可达，且 PERF 双例在**任何机器**修 spec 前恒红——**F3 复跑门必须在标准 e2e 云机执行，且以「84 − 2 规格恒红 = 82 例 0/0/0」为过门口径**（或 CC-PERF 工单先落地则 84 全量）。跑法：`E2E_PORT=4399 pnpm exec playwright test`（webServer 自拉 preview 于 4399，与共享 checkout 的 4321 零接触）。
...
7. **CITY-PERF-01/02 恒红勿追跑**：败点是规格前提失实（断言的 DOM 挂点在被测页全史不存在），任何 VM、任何树重跑都不会转绿——空窗重跑只花 9 分钟即可定性（①–④ 步全过 + 败点恒同），别按「竞争嫌疑」反复排窗。通则：**规格声称的取证挂点必须 `rg dist/` 实证**，「两页共用装配段」这类推断不能替代对页壳 HTML 的直接检查。
8. **OBS-06 禁单跑/禁过滤跑**：其消费同 session 内 OBS-01/01b 产出的 dump 工件，新 session 会清空 `test-results/`——闭账必须三例同 invocation 连跑（R4 实证 34ms 即绿）。
...
| F3 | 复跑：CITY-AUD-01 + CITY-NAV-01/02/03 定向轮 + 全量 0/0/0（分母以届时 fresh `--list` 为准；**过门口径 = 84 − 2 规格恒红 = 82 例**，CITY-PERF-01/02 归 CC-PERF 规格工单、非 #166 欠账——若该工单先落地则回归 84 全量）；在标准 e2e 云机执行（本 VM 环境四项不可达，§2.1） | 过门即 GO |

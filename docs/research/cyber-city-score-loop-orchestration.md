...
`main` @ `483b942` · 2026-08-28 08:33 UTC · **R3+§F 收账：#146 功能 87 合入落账 + 双 ENV 处置（§F 翻案后：旧僵尸除名 / 新免责改判）+ ×2 改锚 run7/run8（SEC-R3-LEDGER，事件驱动界点单，r2 修订）** · 范式 `cyber-city-orchestration-paradigm.md` · 本单 = 董事会 R3（[#151](https://github.com/rayw-lab/website/pull/151)，含 §F Tick#32 增量终裁 = 最终口径）§E-4/§F5-5 看板收账件（base = main@`483b942`，只改看板单文件、零 src）
...
>
> **在途等待项（§F 后现行口径）**：① X2 链路——**#129 ×2 门改锚 run7/run8 同机决定趟**（§F F2-1/F3，跑道主 = 指挥官 VM；**基线 v2 = `00fd832` 冻结**）：run1/run2 诊断不计（㉓㉔）· run3 07:16✓ 降级不计（T29 [#150](https://github.com/rayw-lab/website/pull/150)）· run4 无点火作废（T26 [#148](https://github.com/rayw-lab/website/pull/148) / R2 [#149](https://github.com/rayw-lab/website/pull/149)）· **run5 α/β 双趟均诊断不计（§F 定谳）、run5 烧毁 run6 退役**；每趟两键文书（fresh 真空三查 + `IGNITION-runN.txt` **原件**唯一有效，F3-3）+ **跑道互斥令**（㉚，按机适用、自跑不豁免）+ **archive-then-clean 铁则**（㉙）+ **收轮即上链**（§B-3：未上链 ✓ 不计 ×2）+ **每趟 ✓ 独立只读复核**（§C-4，防自跑自判）；假 ✓ 机制实证在册（㉛）；**run7 在飞**（08:28 两键起飞 @`00fd832`）；✓✓（run7+run8）→ 签字门（扩大清单含 #134 三 spec，T19 §2.3）→ 合流（恒等式续期；指挥官裁量注记 F3-4 在案：东线已三重通过 07:16✓/α✓/β✓，剩余为程序账）；#129 draft 挂双门不变；② **plug 门后件**：[#134](https://github.com/rayw-lab/website/pull/134) IDLE draft（tip `e03271f`，A 案几何内容留段末审计放行，过门后先入 #104 分支）；[#104](https://github.com/rayw-lab/website/pull/104) 维持 draft 禁 ready，复活门三条 = #129 双门 + R2 双清 + 全量 80 例 0/0/0（集成树口径，T19 §3.3）；③ R2 零跑道首批已交付 [#145](https://github.com/rayw-lab/website/pull/145)（draft，㉕ 拖欠账落地清偿）；同窗顾问件 T22 [#141](https://github.com/rayw-lab/website/pull/141) · T24 [#144](https://github.com/rayw-lab/website/pull/144) · GOV-ROLES [#142](https://github.com/rayw-lab/website/pull/142) 待存档波；④ **存档波**：docs-only 顾问/审计 PR 每波 ≤5 批量合并，空档执行、不占 run 窗；本轮序 = **#149 → #150 → #148 → #151（含 §F）**，本单 [#152](https://github.com/rayw-lab/website/pull/152) 可并入候选（R3 §E-5 / F5-5，合并权在父代理/指挥官）；⑤ 全量 e2e 解锁链六条中「#103+塌栈」一项**已落袋 ✅**，关键路径剩 #129 的 ×2 链（**run7/run8 同机**，run7 在飞），开闸 = ✓✓（上链+复核）→ 签字门。
>
...
| 在途 | 内容 | 状态 |
|------|------|------|
| #129 修复段（跑道 = 指挥官 VM 同机，§F） | [#129](https://github.com/rayw-lab/website/pull/129)（EXP-01 途径点改线东侧净道 + BL1 桩排 collider 减深 + 过弯/倒车限速鲁棒化；双 ENV 处置见 R3/§F 节） | **转合流门**：draft · tip = **基线 v2 `00fd832`**（F3-1 措辞修正 + β 三证上链 `e99a451`，零 src 漂移）· 挂双门（**×2 = run7/run8 同机决定趟 0/2**，每趟 ✓ = 两键原件 + 三证上链 + 独立复核 + 测试面解冻指挥官签字门）；run1–run4 与 run5 α/β 均不计；**run7 在飞**（08:28 两键起飞）；过门即合，**先于 plug rebase** |
...
| 待派 | 内容 | 条件 |
|------|------|------|
| run7 收轮三件（在飞决定趟候选，§F F5-4） | run7 已 08:28 两键起飞（IGNITION-run7 原件 + vacuum PASS，tip `00fd832`）——收轮后：① 三证 durable 上链 `docs/research/exp01-evidence/run7/`；② 独立只读复核派单（复核上链三证与 `RUN_EXIT=0` 一致性 + 放行令原件四要素）；③ 两件齐 → run7 计 ✓（×2 → 1/2） | run7 收轮即办（硬闭点 = 点火 +65min 与 `RUN_EXIT=` 取先；在飞窗互斥令生效） |
| run8 决定趟（同机） | `IGNITION-run8.txt` 原件另签（逐趟核发）+ fresh 真空三查档 + 主机指纹行；收轮即上链 `run8/` + 独立复核 | run7 计 ✓ 后 |
| run5-α 证据抢救（§F F5-3） | 单次窄域 resume 新 ENV：仅「本 VM `run5-decisive/` + vacuum/IGNITION 副本 commit 为 evidence-only 提交至 #129 分支 `docs/research/exp01-evidence/run5-alpha/`，零代码零其他动作」；失败即登记灭失（孤证规则维持：α 细节须 β 或 run7 交叉验证后方可引用） | 空档执行（不占 run7 窗） |
| 存档波（R3 §E-5 / F5-5） | 本轮序 = **[#149](https://github.com/rayw-lab/website/pull/149) → [#150](https://github.com/rayw-lab/website/pull/150) → [#148](https://github.com/rayw-lab/website/pull/148) → [#151](https://github.com/rayw-lab/website/pull/151)**（含 §F）→ 本单 [#152](https://github.com/rayw-lab/website/pull/152) 可并入候选；存量候选 #141/#142/#144/#145/#147 及 T6–T21 后续波次（每波 ≤5） | 空档执行、不占 run 窗（合并权在父代理/指挥官，本单不代合） |
...
| PR | 状态 | 说明 |
|----|------|------|
| [#152](https://github.com/rayw-lab/website/pull/152) | draft · 本单（存档波候选可并入） | 秘书 SEC-R3-LEDGER：R3+§F 收账（#146 落账核对 + 双 ENV 处置上板 + ×2 改锚 run7/run8 + 立法上板 + 存档波序） |
| [#151](https://github.com/rayw-lab/website/pull/151) | draft · 存档波第 4 位 · tip `00f6c7c` | 董事会 R3 终裁 + **§F Tick#32 增量终裁（最终口径）**：旧僵尸除名维持 / 新 ENV 免责改判 + 同机强制令 + 放行令原件唯一性 + 工单三纪律 + 生命征象立法 + run5 烧毁/run6 退役 + ×2 改锚 run7/run8@基线 v2 + 禁换将² + #146 转收账 |
...
| [#129](https://github.com/rayw-lab/website/pull/129) | draft · **×2 = run7/run8 同机决定趟（0/2）· run7 在飞** | 修复段（EXP-01 改线东侧净道 + BL1 桩排减深 + 限速鲁棒化）· tip = **基线 v2 `00fd832`**（F3-1 措辞修正 + β 三证上链 `e99a451`，零 src 漂移）· run1–run4 与 run5 α/β 均不计（诊断/降级/作废/§F 定谳）· 双门 = ×2 ✓✓（每趟两键**原件** + 三证上链 + 独立复核）+ 测试面签字（扩大清单含 #134 三 spec）· **过门即合，先于 plug rebase**（恒等式续期） |
...
- **[#129](https://github.com/rayw-lab/website/pull/129) 双门（§F 后现行口径）** · **验证门（×2 = run7/run8 同机决定趟，0/2，run7 在飞）**：run1/run2 诊断趟不计（㉓㉔）· run3 07:16✓ 会计定谳降级不计（T29）· run4 无点火作废（T26/R2）· **run5-α 免责跨 VM 诊断趟（情报账全额/资格账零）· run5-β 降级诊断趟（`run5-beta-diagnostic-0803`，三证已上链 `e99a451`）——run5 烧毁 / run6 退役（§F 定谳）**；每趟 ✓ 要件 = 两键文书（fresh 真空三查 + `IGNITION-runN.txt` **原件**四要素，逐趟核发，F3-3）+ **全程互斥令**（㉚，按机适用、自跑不豁免）+ archive-then-clean（㉙）+ `1 passed/0/0/0` + **三证上链**（§B-3）+ **独立只读复核**（§C-4）；**基线 v2 = `00fd832` 冻结**（#146 docs-only 不触基线，无需 rebase）· **签字门**：测试面四处 + BL1 src + 合流许可，一次性签字（扩大清单含 #134 三 spec，T19 §2.3）· 未过门禁合；过门即合且**先于 plug rebase**（合流窗恒等式续期；指挥官裁量注记 F3-4 在案）
- **执行力曲线**（「裁决链完整、执行链断裂」专项，㉒㉕㉖㉜）· **五账清偿**：#103 复读账（㉖）✅ 合流 `e604568` · #135/#140 塌栈账（㉜）✅ 合流 `9177dcc` · R2 派单账（㉕）✅ 交付 #145 · 归档代办 ✅ 销案（㉑）· #146 上板账 ✅ 合入 `483b942`（08:01:14Z）——余账：**父代理训诫三笔（§F F1-3 新增）**：工单条款越权改写两键制 · 保留字染色 · 双轨同标签致 run5 双占用 + 基线 ×2 窗内被烧（编排权不解除）· 清场令残项**事实消解**（β vacuum @08:03 PASS 采信为运行面证据，F2-1；档面核对随独立复核并案）· ENV 三前置三次跳门（㉔，随 §F 免责改判封账——α 执行质量「无可指摘」在案）· 探针并发运营（主体 = 父代理，㉚）· TRIAGE 董事会终止定谳（禁 resume）

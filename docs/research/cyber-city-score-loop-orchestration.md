...
`main` @ `467d148` · 2026-08-28 09:50 UTC · **R4 合流收口：#129 已合 main@`4f445e4`（×2 = 2/2 ✓✓ + 指挥官明示同意 + 站立授权）+ 存档波 41 单全量收编（波尾 = 站立授权 [#159](https://github.com/rayw-lab/website/pull/159) → `467d148` = tip）+ R5 产品波次上板（SEC-R4-MERGE-CLOSE，事件驱动收口单）** · 范式 `cyber-city-orchestration-paradigm.md` · 本单 = 站立授权规程下秘书收口件（base = main@`467d148`，只改看板单文件、零 src）
...
>
> **在途等待项（R4 收口后现行口径）**：① **下一棒 = 工程双件**——[#134](https://github.com/rayw-lab/website/pull/134)（IDLE draft，栈 base = facade-r2 @`c24c7f3`、tip `e03271f`，A 案几何留段末审计）两步走 = 段末审计放行 → 先入 #104 分支 → 随 #104 单次 rebase 取 ENV canonical（EXP-01 已随 #129 落 main）；[#104](https://github.com/rayw-lab/website/pull/104)（draft 禁 ready，tip `c24c7f3`）复活门余两条 = R2 双清 + 全量 80 例 0/0/0（「#104 候选 ⊕ main 集成树」口径，T19 §3.3）——**#129 双门一条已落袋 ✅**；全量 e2e 窗需专项排程（互斥令适用）。② **R5 调研先行四单待派**（音效 / M 小地图 / 视角旋转 / BGM+缩略条，实现门禁六门八禁见 [#158](https://github.com/rayw-lab/website/pull/158)）——视角旋转实现须**指挥官红线签字**；音效单可引用音频调研既有结论（[#157](https://github.com/rayw-lab/website/pull/157) 已合 `3b7498e`）。③ **真机线**：S-2 kit [#108](https://github.com/rayw-lab/website/pull/108) 已随存档波合 main、性能六腿 [#96](https://github.com/rayw-lab/website/pull/96) 在 main，仍待指挥官启动 → AL-PERF / AL-R10 → COMP-M0 五维重算。④ **run5-α 证据抢救**（§F F5-3 单次窄域 resume，evidence-only）落点改 main 存档 PR（#129 已合），空档执行、失败即登记灭失（孤证规则维持）。⑤ **存档波已清空 docs 积压**（41 单，MERGE-WAVE 10）；OPEN PR 仅余工程双件 + 冻结另案，后续 docs 件按站立授权规程随到随裁。
>
...
| **视觉** | **98** | **73** | +25 | 顾问路径 →~78（[#98](https://github.com/rayw-lab/website/pull/98) 已合）· X1b [#101](https://github.com/rayw-lab/website/pull/101) 已合待复评 · **[#129](https://github.com/rayw-lab/website/pull/129) 已合 main@`4f445e4`（×2 = 2/2 ✓✓ 闭合，双门销案）** → 下一棒 plug [#134](https://github.com/rayw-lab/website/pull/134) 段末审计 → [#104](https://github.com/rayw-lab/website/pull/104) 复活门余两条 + 全量 e2e 排程 · R5 产品波次调研先行（[#158](https://github.com/rayw-lab/website/pull/158)） |
...
## 当前焦点（提分 Loop · R4 合流收口后）

| 轨 | 状态 | 下一拍 |
|----|------|--------|
| 视觉 | **73** ✅ [#94](https://github.com/rayw-lab/website/pull/94) · X1b 已合 [#101](https://github.com/rayw-lab/website/pull/101)（W2③）· **[#129](https://github.com/rayw-lab/website/pull/129) 已合 `4f445e4`**（×2 run7 ✓ [#154](https://github.com/rayw-lab/website/pull/154) + run8 ✓ [#156](https://github.com/rayw-lab/website/pull/156) + 明示同意 + 站立授权，双门销案） | 段末审计对 [#134](https://github.com/rayw-lab/website/pull/134) A 案几何放行 → #134 入 #104 分支 → [#104](https://github.com/rayw-lab/website/pull/104) rebase 取 ENV canonical → 复活门余两条（R2 双清 + 全量 80 例 0/0/0 集成树）→ 复评冲 ~78；R5 产品波次调研先行四单（音效 > 小地图 > 旋转[红线签字] > BGM > 缩略条，[#158](https://github.com/rayw-lab/website/pull/158)） |
...
| 事件驱动（R4 收口窗） | 站立授权 [#159](https://github.com/rayw-lab/website/pull/159) + 存档波 MERGE-WAVE 10 + R5 上板 + 秘书 SEC-R4（本单） | #159 MERGED → **`467d148`**（合流裁决规程落 main：docs 直合 / src 董事会急裁 / 真机六腿·北极星永不代决）· 存档波 41 单全量收编（`483b942`→`467d148`）· R5 [#158](https://github.com/rayw-lab/website/pull/158) → `9c5f102`（产品波次 + 性能声明分账）· 本单收口 |
...
| 在途 | 内容 | 状态 |
|------|------|------|
| plug 交付段（下一棒①） | [#134](https://github.com/rayw-lab/website/pull/134)（栈① base=facade-r2 @ `c24c7f3`，tip `e03271f`）：桥位南移 + 东北簇内退 + 探针走廊余量审计 | **IDLE · draft**；A 案几何留段末审计（内容与纪律分账，纪律事件 #3 在册 ⑱）；两步走 = 段末审计放行 → 先入 #104 分支 → 随 #104 单次 rebase 取 ENV canonical（EXP-01 已随 #129 落 main，T19 §3.2 / #145 预案） |
| X2 实现段（下一棒②） | [#104](https://github.com/rayw-lab/website/pull/104) tip `c24c7f3` | **draft 禁 ready**；复活门余两条 = R2 双清 + 全量 80 例 0/0/0（跑「#104 候选 ⊕ main 集成树」，T19 §3.3）——**#129 双门一条已落袋 ✅**；需 rebase + 全量 e2e 专项排程（互斥令适用） |
| R5 调研先行四单 | 音效（可引 [#157](https://github.com/rayw-lab/website/pull/157) 音频调研）/ M 小地图 / 视角旋转 / BGM+缩略条（实现门禁六门八禁，[#158](https://github.com/rayw-lab/website/pull/158)） | **待派**（调研先行、实现后置；视角旋转实现须指挥官红线签字） |
| SEC-R4-MERGE-CLOSE | 本单：R4 合流收口登记（#129 落账 + ×2 闭合 + 站立授权 + MERGE-WAVE 10 + R5 上板；base = main@`467d148`，只改看板单文件） | 本单 |

plug 验证轮已收轮（终局 ⑰）；plug r1 `368b4d4`/`839b6fe` **冻结不 revert 不扩** + 禁改动线令维持；残波事件收尾（⑳㉒）。TRIAGE bc-ace126a4 **董事会终止定谳禁 resume**（终裁 §1）。

已收口（不再占槽）：**#129 修复段 ✅ `4f445e4`**（×2 run7/run8 ✓✓ + 明示同意 + 站立授权，双门销案）；**×2 审计双单 ✅**（[#154](https://github.com/rayw-lab/website/pull/154) `ac8033c` / [#156](https://github.com/rayw-lab/website/pull/156) `85423f8`）；**站立授权 ✅**（[#159](https://github.com/rayw-lab/website/pull/159) `467d148`）；**存档波 41 单全量收编 ✅**（董事会 R2/R3/R4-PERF/R5 · T29 · SEC-R3 · 音频调研 · 顾问 T2–T22 · 落库审计 · GOV-ROLES · R2 首批 [#145](https://github.com/rayw-lab/website/pull/145) · kit 件，全落 main——见 MERGE-WAVE 10）；[#153](https://github.com/rayw-lab/website/pull/153) CLOSED superseded；前序 **#103 合流落库 ✅**（`e604568`）；**#143 董事会件 + 塌栈 ✅**（`9177dcc`）；**#146 功能 87 上板 ✅**（`483b942`）；**plug 验证轮**（终局 1/2/0/0 ⑰）；**归档代办落地销案**（㉑）。

| 待派 | 内容 | 条件 |
|------|------|------|
| 段末审计 AL（plug/facade） | [#134](https://github.com/rayw-lab/website/pull/134) A 案几何放行裁决 + 「#104 候选 ⊕ main」集成树全量 80 例 0/0/0 复跑排程 | #104 rebase 后派；全量窗互斥令适用（审计零业务代码） |
| R5 调研先行四单 | 音效（引 [#157](https://github.com/rayw-lab/website/pull/157) 既有结论）/ M 小地图 / 视角旋转 / BGM+缩略条 | 槽空即派（实现门禁六门八禁 [#158](https://github.com/rayw-lab/website/pull/158)；旋转实现须指挥官红线签字） |
...
## OPEN PR（快照 2026-08-28 09:50:13Z——存档波后仅余 11 单）

| PR | 状态 | 说明 |
|----|------|------|
| [#134](https://github.com/rayw-lab/website/pull/134) | draft · **下一棒①** · 栈 base=facade-r2 | plug 交付段（tip `e03271f`）：桥位南移 + 东北簇内退 + 探针走廊余量审计；A 案几何留段末审计（纪律事件 #3 在册 ⑱，内容与纪律分账）；放行后先入 #104 分支、随 #104 单次 rebase 取 ENV canonical |
| [#104](https://github.com/rayw-lab/website/pull/104) | draft · **下一棒②** · 禁 ready | X2 立面套件（W2④，tip `c24c7f3`）：复活门余两条 = R2 双清 + 全量 80 例 0/0/0（「候选 ⊕ main」集成树，T19 §3.3）——#129 双门一条已落袋 ✅；需 rebase + 全量 e2e 专项排程 |
...
docs 积压已由存档波清空（41 单，MERGE-WAVE 10）；此后 docs 件按站立授权规程（#159）随到随裁，不再堆积。
...
- **[#104](https://github.com/rayw-lab/website/pull/104)** · **禁 ready** · 复活门**余两条** = R2 双清 + 全量 80 例 0 failed/0 skipped/0 flaky（跑「#104 候选 ⊕ main 集成树」，T19 §3.3）——第一条 [#129](https://github.com/rayw-lab/website/pull/129) 双门**已落袋 ✅**（`4f445e4`）；rebase 取 ENV canonical + 全量窗专项排程为前置
...
- **全量 e2e 解锁链**（T19 §4.2 / T20 §3.2 / T21 §4 六条）· 「#103+塌栈」✅ + **#129 ×2 链 ✅ 落袋**（run7/run8 ✓✓ → 签字门 → 合入 `4f445e4`）；开闸残余 = [#134](https://github.com/rayw-lab/website/pull/134) 段末审计 + [#104](https://github.com/rayw-lab/website/pull/104) rebase 后「候选 ⊕ main」集成树全量 80 例专项排程；互斥令同样适用于全量窗

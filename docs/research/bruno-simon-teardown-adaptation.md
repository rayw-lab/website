...
| 预算门禁 | 懒加载 JS ≤ 400KB gzip（three 已有 256KB 基线 + 控制逻辑）；资产 ≤ 3MB（复用现有 CarConcept 3.5MB 的 Draco 重压缩版，目标 ≤ 2MB） |
...
### 10.3 master-plan 修订（1 处，搭既有便车）

| # | 章节 | 建议修订 |
|---|------|---------|
| M1 | 第 6 章视觉风格（动效行） | SRD R4 行动项本来就要求 Phase 2 前修订此章（Hero 实时渲染 + 循环动画配额豁免）；**同一次修订顺带加入第三条豁免**：「独立路由的沉浸式展项（/world/）不受『动效仅 hover 与渐入』约束，受 SRD §12.7 运行时预算约束」——避免二次修订总纲 |

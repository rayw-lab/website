...
2. e2e 全量（隔离端口 `E2E_PORT=4340`，防 4321 dev server 串台）→ `test-results/e2e-results.json`
3. LHCI：SwiftShader 本 VM null → 同 SHA green CI artifact 回填（登记来源 commit + run id；ci.yml ⑤ `treosh/lighthouse-ci-action` `uploadArtifacts: true`）

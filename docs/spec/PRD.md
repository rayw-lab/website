...
| 上游输入 | `docs/website-plan/master-plan.md`（建站总纲）、`docs/website-plan/mvp-checklist.md`、`docs/website-plan/positioning-onepager.md`、`docs/website-plan/case-outlines.md`、`docs/website-plan/material-security-grading.md`、`docs/research/homepage-redesign-spec.md`、`docs/research/portfolio-inspiration-community.md`、`docs/research/portfolio-inspiration-index.md` |
| 冲突裁决 | 定位与红线以 `master-plan.md` 为准；本 PRD 负责产品范围、功能清单与路线图。范围冲突时先修订本 PRD 再动工 |
...
| 判断力证明 | 无 | 每个案例的「核心判断/取舍/失败复盘」模块 + Insights 可证伪观点 |
...
| 深度验证 | 12 模块正文、证据徽章、「配套 Demo →」挂钩 | 读者在「取舍/失败复盘」模块形成「这人真做过」的判断；Demo 一次点击可达且秒开 | 数据无出处→证据等级如实标注；Demo 加载无反馈→进度指示+30 秒结论+跳过出口 |
| 延伸 | 模块 12 相关链接、RSS、Lab 索引 | 单次访问 ≥ 3 页；7 日回访 | 内容孤岛 → 每篇案例强制 ≥ 2 条站内互链（产品规则，见 §7.3） |
...
| WORK-02 | 案例详情 12 模块模板 | MDX 固化 12 模块（一句话摘要→背景→约束→角色→核心判断→方案路径→关键产物→结果→取舍→失败复盘→可复用方法→信息边界），顺序不变、可按敏感度裁剪 | P0 | MVP | Work / 内容治理 | 模板经 schema 校验；「待填」项未清零的案例构建时阻断发布 |
...
| WORK-07 | 旗舰案例 B/C 精简版 | 「端云大模型分层」「AI 原生工作流」先上模块 1/2/6/9/12 精简版，V1 补全 12 模块 | P0 | MVP（精简）→ V1（全量） | Work | 精简版五模块完整；页内明示「完整版更新中」但无 Coming soon 空区块 |
...
| GLB-03 | RSS 订阅 | 全站 RSS（work/insights/ai-lab 三 collection 合流），Footer 与 Insights 页入口 | P0 | MVP | 全局 / Insights | RSS 校验通过；条目含全文摘要与 canonical 链接 |
...
| GOV-05 | 信息边界声明模块 | 每篇案例模块 12 明示「哪些信息做了处理、以什么方式处理」——脱敏透明化本身建立信任 | P0 | MVP | 内容治理 / Work | 三旗舰案例 100% 含该模块 |
| GOV-06 | 内容修订与时效管理 | `updatedDate` 字段 + 时效性强的结论标注版本/日期；季度盘点过期内容 | P1 | V1 | 内容治理 | 修订历史可追溯；AI 相关结论 100% 带写作时间 |

**功能需求合计：65 条**（HOME 9 + WORK 10 + INS 6 + LAB 15 + ABT 6 + GLB 13 + GOV 6）。
...
**排期逻辑**：V1 先做 1–3 号（每支柱一个，与三旗舰案例一一配对，补全「每个主张都有可交互证据」的产品承诺）；V2 按数据反馈（Lab 页停留、转发、由 Lab 带来的联系）从 4–8 号中择优推进，**成熟一个上线一个，不设死排期**。所有 Concept 态构想仅在 Lab 索引页以卡片预告（LAB-07 状态徽章），不生成空页。
...
| 4 | Demo 参与度 | Demo 页交互触发率（播放/换色等首次交互）与案例→Demo 转化率 | 交互率 ≥ 40%；转化 ≥ 15% | Demo 内自定义事件 |
...
| 与岗位无关的通用炫技（粒子背景、赛博朋克主题、3D 头像、流体模拟） | 触发「AI 味模板脸」与「炫技与岗位无关」双反模式；本站炫技白名单只有领域强相关项（车模/座舱 HMI/多语种/工作流可视化） |
| 平滑滚动库/滚动劫持/开屏动画 | 社区反模式烈度第一梯队；与「10 秒定位」目标直接冲突 |
...
| 2 | **过度炫技**：Hero 3D、多个 Lab 把站点拖成「加载 30 秒的玩具」，触发社区最高烈度反模式 | 中 | 高 | 性能门禁硬阻断（GLB-08）；炫技白名单 = 领域强相关项；重型演示永远内页化 + 30 秒结论 + 跳过出口（LAB-05）；循环动画配额 ≤ 2 处；HOME-07 设有明确降级触发条件 |
...
| bruno-simon.com | 3D 炫技天花板 | ❌（被 3D 绑架） | ❌ | ✅（网站即作品） | ❌ | ✅（他卖的就是 WebGL） | ✅ | 炫技只在「炫的=卖的」时成立；反面教材：加载与信息可达性 |

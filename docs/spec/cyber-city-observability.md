...
| ux | `world-bgm` | `{enabled, source: 'user' \| 'restore'}` | 显式 · `audio/BgmLoop.ts`（`[data-world-bgm]` BGM 钮切换 → `'user'`，enabled = 切换后开关态；解锁沿 localStorage `world-bgm-on='1'` 记忆自动恢复 → `'restore'`——恢复恒晚于解锁；默认 OFF 无事件（禁项③））。[CC-BGM-C1] 随行加法：合成氛围垫 v0 可观测面（董事会急裁 bgm-synth-scope §C 文件域 + cc-bgm-rs §4.1 DP-B1 新 type），schemaVersion 不动 | P0 |

...
| 3 | CarConcept Garage 沿街 hero 楼实模 + 南前场道具簇 GLB（Blender 4.0 headless 全程序化生成 + Draco + KTX2/ETC1S，CC-BL2） | `public/models/concept-garage/ConceptGarage.glb` | 本仓脚本 `tools/blender/generate-concept-garage.py`（确定性 seed=0x2207，2026-08-27 生成；零外部网格/贴图，无 .blend 二进制入库） | **原创**（随仓库许可；零第三方义务） | **146,464 B（≈143 KB）** | 非首包（Q0/Q1 城市挂载后异步流式拉取；Q2 挂载零字节零请求） | 单 GLB ≤10MB spike 合同 ✅（2,586 tris ≤100k、贴图 3 张 KTX2 ≤1024²≤2K）；world 流式池 ≤12MB（`audit-budget` G-G(world) 直测目录）✅ |

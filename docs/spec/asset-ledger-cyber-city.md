...
| 3 | CarConcept Garage 沿街 hero 楼实模 + 西端天际线段 + 南前场道具簇 GLB（Blender 4.0 headless 全程序化生成 + Draco + KTX2/ETC1S，CC-BL2 / CC-BL2-R2 体量再分布重生成） | `public/models/concept-garage/ConceptGarage.glb` | 本仓脚本 `tools/blender/generate-concept-garage.py`（确定性 seed=0x2207，2026-08-27 重生成；零外部网格/贴图，无 .blend 二进制入库；SHA-256 `0b3717d461acd6d26d2d9725361a4b2df8439204b3c59557a82eeca25d3c35fb`，审计复现 = 同脚本 + `gltf-transform etc1s --quality 255` + `gltf-transform draco` 字节级一致） | **原创**（随仓库许可；零第三方义务） | **148,696 B（≈145 KB）** | 非首包（Q0/Q1 城市挂载后异步流式拉取；Q2 挂载零字节零请求） | 单 GLB ≤10MB spike 合同 ✅（2,996 tris ≤100k、贴图 3 张 KTX2 ≤1024²≤2K）；world 流式池 ≤12MB（`audit-budget` G-G(world) 直测目录）✅；视觉包络：主体 h18 同笼 + 西端上探段体量再分布（肩块 21.6 不变/南立面西段鼓塔 22.9/桅杆信标顶 24.65——AL-BL2 §8.1 上探许可延续，R2 迁入 `?poi=work-gallery` 帧内可读区；物理碰撞 h18 合同不变） |
| 4 | Voice Pod「座舱语音舱」第三栋 hero 楼实模 GLB（Blender 4.0 headless 全程序化生成 + Draco + KTX2/ETC1S，CC-VIS-X1B——设计确认 §3 W2③ 选楼裁定见 `public/models/voice-pod/README.md`） | `public/models/voice-pod/VoicePod.glb` | 本仓脚本 `tools/blender/generate-voice-pod.py`（确定性 seed=0x2D6F，2026-08-27 生成；零外部网格/贴图，无 .blend 二进制入库；SHA-256 `56c96b24583c2d10d7eb469d2a1689fbf3261c25267a70896db46a9abce3ea30`，审计复现 = 同脚本 + `gltf-transform etc1s --quality 255` + `gltf-transform draco` 字节级一致） | **原创**（随仓库许可；零第三方义务） | **170,068 B（≈166 KB）** | 非首包（Q0/Q1 城市挂载后异步流式拉取；Q2 挂载零字节零请求） | 单 GLB ≤10MB spike 合同 ✅（3,054 tris ≤100k、贴图 3 张 KTX2 ≤1024²≤2K、≤300KB/栋批预算 ✅）；world 流式池 ≤12MB（`audit-budget` G-G(world) 直测目录）✅；视觉包络 footprint w32×d32×h42 **严格同笼**（脚本包络自检 assert：\|x\|≤16.33/\|y\|≤16.34/z≤41.75；零基座外挑、零随楼道具、零新增碰撞体，引擎零改动） |
| 5 | 城区立面套件 + 街角道具带 + 前景景框构件库 GLB（Blender 4.0 headless 全程序化生成 + Draco + KTX2/ETC1S，CC-VIS-X2-FACADE-R2） | `public/models/facade-kit/FacadeKit.glb` | 本仓脚本 `tools/blender/generate-facade-kit.py`（确定性 seed=0x2FAC，2026-08-27 生成；零外部网格/贴图，无 .blend 二进制入库；SHA-256 `f5e25ef8549d6f4823fde078fa4711d1ed039a770cbaa5d8ca352a4edcf6276c`，审计复现 = 同脚本 + `gltf-transform etc1s --quality 255` + `gltf-transform draco` 字节级一致） | **原创**（随仓库许可；零第三方义务） | **80,024 B（≈78 KB）** | 非首包（Q0/Q1 城市挂载后异步流式拉取；Q2 挂载零字节零请求） | 单 GLB ≤10MB spike 合同 ✅（1,474 tris ≤100k、贴图 2 张 KTX2 base 1024²+emissive 512² ≤2K）；world 流式池 ≤12MB（`audit-budget` G-G(world) 直测目录）✅；形态差异：**模块化构件库**非单楼实模——10 具名 mesh 单材质 `KitSurface`，运行时 InstancedMesh 贴附 NDC 取证在册的可见临街面（消费方 CityBlocks/StreetProps/ForegroundFraming，全套 ≤10 draw call）；emissive 全部 ≤1 阈下零新增辉光锚 |

改造与复现细节：`public/models/hero-robot/README.md`（改造清单、热替换约定、失败回退）；管线脚本全文见附录 A。autodrive-lab / concept-garage / voice-pod / facade-kit 生成/压缩管线、材质名合同与回退合同见各自 `public/models/*/README.md`（复现命令三行：blender headless → `gltf-transform etc1s` → `gltf-transform draco`；四件同管线，BL2 差异项 = 身份色 `AccentBlue/BeaconBlue`、seed 0x2207、无柱悬挑雨棚泊车圈让空；X1B 差异项 = 身份色 `AccentPink/BeaconPink`、seed 0x2D6F、穿孔声学板/消声劈尖贴图、波形天冠、零道具碰撞体；X2 差异项 = 构件库形态、seed 0x2FAC、NDC 可见楼清单先行 + 跨楼共享零身份色——见 facade-kit README §NDC/§碰撞）。

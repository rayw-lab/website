...
改造与复现细节：`public/models/hero-robot/README.md`（改造清单、热替换约定、失败回退）；管线脚本全文见附录 A。autodrive-lab / concept-garage / voice-pod 生成/压缩管线、材质名合同与回退合同见各自 `public/models/*/README.md`（复现命令三行：blender headless → `gltf-transform etc1s` → `gltf-transform draco`；三楼同管线，BL2 差异项 = 身份色 `AccentBlue/BeaconBlue`、seed 0x2207、无柱悬挑雨棚泊车圈让空；X1B 差异项 = 身份色 `AccentPink/BeaconPink`、seed 0x2D6F、穿孔声学板/消声劈尖贴图、波形天冠、零道具碰撞体）。

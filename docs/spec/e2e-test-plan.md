# 浏览器 E2E 测试计划（Batch 1）

> 状态：已落地（分支 `cursor/e2e-testing-1d6f`）· Batch 1 执行结果见 [e2e-test-report-batch1.md](./e2e-test-report-batch1.md)
> 被测对象：Astro 静态站（GitHub Pages 项目页，base `/website`）+ Lab 子系统（统一 facade + WebGL/WebGPU 模块）
> 上位规格：SRD §9.2（facade 契约）/ §12.4（降级链）/ §11.2（CI 门禁）；homepage-redesign-spec §3.1 / §8.4
...
|---|---|---|---|---|---|
| 加载/骨架/SEO 基础 | HOME-01 | LAB-01 | TTS-01 | CAR-01 | SITE-04* |
| 导航链路 | HOME-02/04 | LAB-03 | —（面包屑随 LabLayout） | 同左 | — |
| 主题切换 + 持久化 | HOME-03 | —（独立暗色静态页） | — | — | — |
| facade poster→canvas | — | — | TTS-01 | CAR-01 | — |
| 深链参数 | — | — | TTS-03/04/05（`?locale= ?scene=`） | CAR-02/03/04（`?paint= ?wheels= ?livery= ?gl=`） | — |
| reduced-motion 阻断 + 显式启动 | — | — | TTS-06 | CAR-06 | — |
| `?gl=1` / 降级链 | — | — | —（无 GL） | CAR-01/02 | — |
| RTL 阿拉伯语 | — | — | TTS-04（+he-IL 角标） | — | — |
| 移动端 375px | MOB-01 | — | MOB-02 | MOB-03（pointer 拦截） | — |
| 断链/404 | SITE-01/02（全站爬取 + 白名单镜像） | 同左 | 同左 | 同左 | SITE-01（白名单）|
| 无 JS noscript | HOME-05 | LAB-04 | TTS-07 | CAR-07 | — |
| sitemap/noindex | — | — | — | — | SITE-03（排除断言） |

\* world-spike：路由在 `check-links` PENDING_ROUTES 白名单内（Track C 未交付）。SITE-04 为条件用例——路由 404 时 skip 并注明；一旦交付自动生效（烟测 + 零未捕获异常），同时 SITE-01 的白名单过期检查会强制清退白名单条目。

**已知待交付路由**（与 `scripts/check-links.mjs` PENDING_ROUTES 同步维护于 `e2e/helpers.ts`，只收缩不增长）：`/work/`×4、`/insights/`、`/ai-lab/`、`/about/`、`/contact/`、`/world-spike/`、`/rss.xml`。头部导航五链接现阶段命中该白名单属预期；页面交付后 SITE-01 反向阻断，强制同步删除条目。
...
- **选择器契约**：优先 facade/模块的 `data-*` 契约属性（`data-lab-host/-state/-blocked/-start/-gated`、`data-cfg-*`、`data-locale/-scene`），不用样式类名做定位（`.scene-btn` 仅为收窄 `#screen[data-scene]` 属性碰撞）。
- **白名单同步**：`e2e/helpers.ts` PENDING_ROUTES 与 `scripts/check-links.mjs` 双向同步；两侧都有过期反向阻断。
...
4. world-spike 交付后的完整交互用例（键盘/触摇杆输入、物理量断言）；
5. 弱网/离线：`context.route` 模拟 mp3/模型 404 与慢链路，断言 facade error 态文案（§9.2 error 契约目前仅代码审查覆盖）；
6. bfcache/View Transitions 离页 dispose 断言（`astro:before-swap` 后 GPU 资源释放，需 `page.goBack` + 内存探针）。

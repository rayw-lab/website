...
### R23-1 🔴 同一个流程坑撞了第二次：补丁脚本失败了，commit 照样跑
两次都是这个形状：`python3 - <<'PY' ... sys.exit('MISS')` 未命中就退出**且不写盘**，
但我把 `git commit` 写在同一条 Bash 的下一行，于是**提交了一个没打上补丁的树**，
而屏幕上「命中不足」那行混在输出里没拦住我。
第一次发生在 R19 勘误（当时补做了），第二次就是 R23——裁决表写进去了，正文没改。
判据：**任何"改文件 + 提交"的组合，提交必须挂在改文件那步的退出码后面**（`patch && git commit`），
不能只是写在它下面一行。这与「门的退出码禁经管道」是同一条：**顺序不是依赖，`&&` 才是。**
更深一层：这两次的症状都是「标注了不一致 ≠ 修正了不一致」——
我写下了裁决，就以为改完了。

## R24 · 文案表双向闭合：把「单向门」补成两侧（2026-09-04 09:0x–09:2x）

### R24-1 glm 点名的那一侧
原来的 fail-closed 只挡「有 slug 无文案」。反过来——**文案表里有一个谁也不认识的键**
（改名后留下的孤儿、或提前写好的下一个厅）——会一直静默待着，
直到某天有人以为它已经接线了。这是「集合扩张必同步具名清单」的**值域侧**：
成员增加肉眼可见，键变孤儿看不见。

已补 `assertHallCopyClosed()`，两个方向都在**构建期**抛错，并把它挪进 `getStaticPaths()`
（每次构建跑一次，而不是每页渲染时才发现）。

### R24-2 逻辑三格先验（构建期接线另验）
拿真实的 `world-halls.json` 喂同一份判据函数：

| 用例 | 结果 |
|---|---|
| ① 正控：现状两侧闭合 | `OK` |
| ② 负控 A：少一条文案 | `THROW missing: agent-nexus` |
| ③ 负控 B：多一个孤儿键 `ghost-hall` | `THROW orphan: ghost-hall` |

🔴 但这只证明了**判据函数本身对**，没证明**它真的在构建时被执行**——
那是两件事（「只能从 main() 到达的规则任何门都咨询不到」的同族）。
构建期接线用故障注入另验，见 R24-3。

### R24-3 顺带记一条卫生问题（非本战役引入）
`car-configurator` 的 e2e 每跑一次就覆写受版本管理的文档资产
...
## R27 · NX-W4 收口：手卷 + 五跋骨架 + 收官（2026-09-04 10:3x–10:5x）

### R27-1 手卷按 ADR-7 落地，三态门进 e2e
桌面：sticky 长区间（480vh），`progress = clamp(-rect.top / (section - sticky), 0, 1)` 驱动 `translateX(-p·dx)`；
不拦截任何事件；首帧即算一次（刷新落中段按当前滚动恢复）。移动端纯竖滚；reduced-motion 不平移、五跋走文档流。
实测（真路由 1440×900）：滚到底 progress **0.978**、平移 **−1252.3px**，与 p·dx = 0.978 × (−1280) 吻合；
375 视口与 RM 下 `transform:none`、sticky 退为 static。三态都写进 e2e。

### R27-2 五跋只交骨架，机器面不冒充定稿
题取草案 §2.2 立意候选并**明标「候选 · 待定稿」**；文/迹占位写明「待磊哥」；`data-status="needs-leige"` 进 e2e 断言。
印严格来自台账：跋④ 8 枚真收据三态（朱文/白文/灰印），其余四跋如实一枚「无收据」灰印。
绑定只到规则**标题级**（`rule:~/.claude/rules/<basename>`），公开尺度逐条待磊哥批；绑定的 receipt id 全部在 `ledger.receipts`（e2e 核）。

### R27-3 我自己看图挑出来的：手卷「内容顶着上沿、下面一片空白」
首版 sticky 高度 = 整个视口，五跋只占上半，是默认布局味。封顶 46rem + strip 垂直居中。
判据：**门全绿不等于构图成立，出图后自己先看一遍再派审**。

### R27-4 预算与门
页内脚本〔勘误：初稿写「两段合计 917B」，是按"页面内联 `<script>` 求和"量的，**看不到被 import 的 chunk、也没把全站脚本剔出去**（glm W7 反核指出）。
按正确口径：展厅自有内联（手卷 472 + 印阵 346 + 收官 425）1,243B + 非引擎外链 2,121B = **3,364B gzip（≤12KB 门）**；引擎 9,265B（≤30KB）。量法已固化为 `scripts/nexus-budget-gate.mjs`（三道子预算分别断言、沿 import 图追 chunk）。〕
`build` rc=0 · `check-links` rc=0 · `about-hall-gate` rc=0 · 墨迹册 e2e **22/22**。全量五 project 整跑本轮另起（上一轮结果随 /tmp 丢失，UNVERIFIED）。


## R28 · 盲审收稿 + 反核裁决 + 手卷重排（2026-09-04 11:0x–11:3x）

### R28-1 agy 六幕盲审 A 段：7.8/10，11 条 finding 逐条亲核
| # | finding | 亲核 | 处置 |
|---|---|---|---|
| P0-1 | 收官首屏露出上一幕「定/清」半截 | **截图伪影**（我按 `top-80` 取景，两按钮本在文档流），但节奏问题属实 | 收官顶部留白 16vh + `min-block-size:60svh` |
| P0-2 | 试墨画布中央出现「这一幕需要 WebGL2」降级文案 | **属实且真因未坐实**：e2e 里试墨是 ready 的；守卫是「挂载 2s 内未步进且在视口」才触发，截图时它在视口外 → 更像 init-failed（多画布争抢）| **待坐实**：全量 e2e 结束后在同一截图流程读回 `data-trial-fallback` 原文再定 |
| P0-3 | 手卷左缘硬切半个字 | 属实 | 两端 4rem→3.5rem 渐隐遮罩 |
| P1-4 | 墨流三条空带顶重底飘 | Grok 空带磊哥令不做；Cursor 14 / agy 10 条是真稀疏 | 不改等高编码（诚实），每带加极淡基线表「量过了」 |
| P1-5 | 印文硬截断 `glm53flas`/`api-direc` | **属实**（`slice(0,9)`） | 语义缩写表，永不切在词中 |
| P1-6 | 印阵与试墨挤同一屏 | 属实 | 幕间留白 20vh |
| P1-7 | 出口按钮圆角实心胶囊风格突变 | 属实（沿用 about 厅样式） | 直角 1px 描边，主出口朱砂线 |
| P2-8 | 导航「Al Lab」大小写错 | **驳回**：源码就是 `AI Lab`，读图误判 | — |
| P2-9 | 墨流轴标过淡过小 | 属实 | .66rem / #4a4f5a / 不透明度 .85；加一行图例 |
| P2-10 | 定/清做成小印落右上 | 采纳 | 朱文「定」白文「清」 |
| P2-11 | 标题横线通栏 | 采纳（W4r 同判） | 去掉，画心天头线是唯一横线 |

### R28-2 glm W7 反核：4 条要补证全部采纳，1 条驳回为空话也采纳
- 「滚到底 p=0.978」自相矛盾——我的探针把"到底"定义成 section 底对齐视口底，而 sticky 封顶后终点更靠后。e2e 改为滚到文档末，**断言 p 必须等于 1**。
- 「tx 与 p·dx 吻合」是循环验证。e2e 改为 **computed matrix 的 tx 与独立量得的 −(scrollWidth − clientWidth) 相等**。
- 五跋断言补硬：`.cf__cand` 可见 ×5；渲染出的朱文/白文印文 ⊆ 台账 receipt id。
- 预算量法盲区：见 R27-4 勘误与 `nexus-budget-gate.mjs`。
- 「先看图再派审」没有载体 → charter 附则 A：视觉派单与 R 条目必须带 `自看：<幕>·<截图>·<一句判断>`，缺项即退回。

### R28-3 agy W4r 手卷调研：整套带数值的改法，采纳
画心 46→31rem、天地界线 + 天大地下（2.6:1.6rem）、题跋面 34→22rem 宽/≥27rem 高（纵势 1.25:1）、
接纸缝替通底网格线、跋间 4rem 留白、章节刻度 + 5rem 滑轨朱砂点替「0%」、五段排版 token（序 .72/题 1.4/文 .95×1.95/迹 .8125/背面 .72）。
...
## R30 · W9 盲评 C 收稿 + 整改 + glm 反核 + LESSONS（2026-09-04 11:5x–12:3x）

### R30-1 agy 盲评 C（`out/W9-agy-blind-C.md`，receipt agy-rescue-20260904-114830-90c54647）：总分 7（六维 7/6/7/8/5/8），13 条
| # | finding | 亲核 | 处置 |
|---|---|---|---|
| P0-1 「5 席位」vs 6 条墨带 | **属实**：`totals.seats`=有会话席位数(5)，Flow 照 6 席名册画（Grok 0 会话） | 只画明细出现的席位；e2e 口径门 `带数 == totals.seats` |
| P0-2 下半四带空 + Claude Code 死黑团 | 空带一半是**采样**（45s 叙事钟才走到 6 成，Cursor 14/agy 10 条在后段）；死黑**属实**（吸收量线性累加后 exp 贴地） | DISPLAY 加渐近软肩 `2.9(1-exp(-a/2.9))`；**2×2**：同流程 45s，近黑(<30)像素 31119→20247（−35%）、纸面均值差 0.0、单滴最深 12→31 |
| P0-3 印阵像 Git 标签 | 部分采纳：阵列 12×2 满阵；印框/九宫格重塑不做（等宽哈希是「机器收据」的诚实形态，改篆刻会假） | 网格断点 12/8/4 |
| P0-4 收官无墨无纸 | 采纳一半：S0 海报的那一滴压在右侧（「墨还在」字面兑现）；全页纸纹属全站壳不做 | v6 首版把题款也带进来了（海报是整幕停帧），v7 用定位把题款推出视口 + 椭圆遮罩 |
| P1-5 导航与纸底硬接缝 | 全站壳 | residual → NEEDS_LEIGE |
| P1-6 横轴无时间刻度 | 属实 | 五个日期锚点，与落墨同公式 `0.07+k/4·0.86`，4% 垂向水线 |
| P1-7 跋四 8 印成发票码 | 属实 | 最多 4 印 + 「共 N 迹」合印，微转错落 |
| P1-8 干区工程虚线圆 | 属实 | box-shadow 水渍环；「纸已干」朱砂 |
| P1-9 定/清贴角、实心红抢戏 | 属实 | 内缩 1.5rem，常态 45%，hover/focus 饱满 |
| P2-10 手卷边缘硬切 | **驳回**：3.5rem 渐隐遮罩在，图 4 左缘「是纸」已是淡出态 | — |
| P2-11 复制钮灰底矩形 | 属实 | 行内文字链 + 朱砂小方印标 |
| P2-12 两个红说不清主次 | 属实 | 只主出口朱砂，回城改墨线 |
| P2-13 底部提示对比低 | 属实 | #33312e |

### R30-2 glm 反核 R29（`out/W9-glm-crosscheck.md`，64k 预算成稿；6k/24k 两次被 reasoning 吃空）
- 5 条：坐实 4 / 不可核 1（e2e、预算数字无产物——正确，反核席本就不该背书）。
- 采纳的保留：①「(0,1,0)」措辞欠准——author 正常声明按 **origin** 压过 UA，与特异度无关，R29-1 该句以本条为准；②Scroll 里「画心封顶 34rem」是陈旧注释，已改；③矮屏 `100svh−header < 36rem` 时 overflow:hidden 仍会裁地脚线，加 `max-height:720px` 放开面高。
- 未采纳：box-sizing 未证（全站 `*{box-sizing:border-box}`，e2e 手卷三态门在跑）；回城覆盖只作用本厅是设计意图。
- 思考预算教训已进 skill（xhsapi/glm 三处 THINKING-BUDGET 段）：`empty content with non-empty reasoning_content` = 预算问题不是失败，≥24k 起、秘书单 48k。

### R30-3 LESSONS（`docs/local-cmd/NEXUS-HALL-LESSONS-2026-09-04.md`）
xhsapi 端点仍 RST，自动换 glm53flash 出稿（receipt api-direct-20260904-115953-7925d6ac，identity_ok）。11 条全带 R 号出处，亲核无编造；「待磊哥」清单与 INDEX 一致。

### R30-4 门与提交
build rc=0；预算门 9785/3768/0/15771 B 四行 PASS（引擎 +286B 是软肩那行）；本厅 e2e **24/24**（新增口径门）；`e8587c0`。全量五项目 e2e 126/131 仍在跑。

### R30-5 自看（v7，dist-next @ e8587c0）
`自看：收官 · /tmp/nx-v7-6.png · 题款鬼影已除，但墨团 .32 几乎看不见，提到 .55；手卷矮屏 · /tmp/nx-v7-3short.png（1280×680）· 地脚线在、跋一完整，glm 遗漏角度 1 已闭合；墨流 · /tmp/nx-flowc-after.png · 团块有了层次但仍偏重，等全量 e2e 后看正式 dist 的海报再定是否再压。`

## R31 · 全量 e2e 收口 + 海报重生成 + seed 盲评 D + 转场立项（2026-09-04 12:2x–12:4x）

### R31-1 全量五项目 e2e（4321，旧 dist @ ≤d70b6c7 构建）
`/tmp/probe/e2e-full3.log` 末行 `131 passed (1.7h)`。⚠️ 它跑的是 W8/W9 改动之前的 dist（跑门中途不重建），只证明接线与其它项目无回归；本厅新改动的全量复跑排在合流前。

### R31-2 收口链（`scratchpad/nx-post-e2e.sh`，每门取 rc）
build rc=0 → 海报重生成（yin 14902→14722 B，flow 同步；软肩改了 S0 停帧）→ 预算门 rc=0 → check-links rc=0 → about-hall-gate rc=0 → ledger-verify rc=0 → ledger-gate rc=0。`ae50d45`。

### R31-3 seed turbo 视觉盲评 D（`out/W9-seed-blind-D.md`，异厂商第二评委）：总分 8，无 P0
| # | finding | 亲核 | 处置 |
|---|---|---|---|
| P1-1 时间刻度只露边 | **采样伪影一半**：1440×900 下墨流画布 16:9 比 100svh 幕高还高，刻度落到视口外；滚动即见 | 宽屏 `aspect-ratio: 2/1` 让整幕一屏放得下（待做） |
| P1-2 手卷缺横向提示 | 部分属实：已有章节指示与滑轨，缺「可横展」的显式提示 | 待做：右缘一枚极淡「→ 横展」 |
| P1-3 操作提示在盲区 | 属实 | 待做：移入画布左下 |
| P2-1 导航栏割裂 | 全站壳（与 agy P1-5 同形） | NEEDS_LEIGE |
| P2-2 席位标签小 | 已 .66rem，再放会压墨带 | 不采 |
| P2-3 收官右半空 | 已用 S0 海报墨团压右侧（seed 看的 v8 是 .32 版） | 已修（.55） |
| P2-4 印阵太规整 | 采纳：±5% 尺寸、±3px 位移的确定性抖动（待做） | 待做 |
| P2-5 款印位置 | 口径型，留 | NEEDS_LEIGE |
| P2-6 纸已干太淡 | 已朱砂；seed 看的 v8 已是新版仍嫌淡 → 提到 #9e2a2b 不透明 | 待做 |
双评并读：agy C 7 / seed D 8；两家都指导航栏割裂与右半留白，属全站壳与已修项。

### R31-4 转场立项（磊哥 12:3x：「操控车子进楼，思考进楼的动效/转场/画面/跳转，要有创意」「创意可以安排 agy 调研，你给引子，让它的创意大于你」）
...
## R34 · agy W11 转场视觉审计 + 三个 P0 整改（2026-09-04 13:4x–14:1x）

### R34-1 收稿（`out/W11-agy-transition-audit.md`，6 张实拍逐帧核）
裁决「炫技及格」，6 帧里判 4 帧不成立。逐条亲核：

| # | agy finding | 亲核 | 处置 |
|---|---|---|---|
| **P0-1** 图5→图6 接力坐标跳 ~7vh、形状 1:1 死圆 vs 1.6:1 扁椭圆 | **属实且有物理根据**：canvas 顶边 y=69 但高仍 900 → 引擎 y=.60 落到视口 **46.8%** 不是 40%。对 S0 成片像素质心实测：**(36.1%, 46.8%)、宽高比 1.48** | 遮罩改 `(36vw, 46.8vh)` + 椭圆 1.45:1；终点不收到 0 而是 7vmax 交棒 |
| **P0-2** 起笔锚点脱落（定帧后车位出画，墨从无特征立面暗部生出） | 属实（`slow-1.png` 车位确已出画） | 起点改为楼底中心经**当前相机**投影 → CSS 变量，机位一变起笔点跟着变 |
| **P0-3** 墨拓是「脏灰蒙版」不是金石拓 | 属实：`contrast(2.4)` 在夜景上只是调暗 | `grayscale(1) brightness(1.7) contrast(5.2) brightness(.86)`——先提亮再高对比，亮部推白成飞白、暗部塌进焦墨。Sobel 未采纳（对 WebGL canvas 卷积强制 CPU 读回，与 0.4s 定帧帧率预算冲突） |
| **P1** 毛边是高斯气泡不是宣纸洇散 | 属实 | feTurbulence 加高频分量（.055/.085）+ 二次位移 scale 13 |
| **P1** 墨幕溢出侵入顶栏 | **误报（采样伪影）**：它看的是收缩到 35% 的定格，此时墨核已小于顶栏高度；t=0 时墨本就满屏 | 不改设计，但顺手把墨幕移出 `.hall-main`（fixed 会被有 filter/transform 的祖先困住） |
| **P2** HUD/遥测未隐 | 一半属实：`#debug` 面板是**我探针引入的伪影**，真 HUD（码表/键位提示）确实还在 | 前奏起帧淡出 `.hud/.hint/.quest/.explore-chip` |

### R34-2 🔴 自己的探针推翻自己：墨幕首帧根本不在场
按 P0-1 改完后顺手验「t=0 是否满屏墨」，探针读到 `--nx-r: undefined` + 整屏纸色——
**墨幕连同 `<div>` 放在 `.hall-main` 中部，HTML 流式解析到那里之前顶栏已经绘制**，
跨文档接驳的白闪期墨幕缺席，「以墨充帘」根本没发生（agy 没看出来，它看的都是中段定格）。
拆出 `ArriveHead.astro` 放 `<head>`：内联脚本抢在首绘前给 `<html>` 挂 `nx-transit`，CSS 同在 head。
逐帧采样实证：**首帧 17.2ms 即 `--nx-r=2448px`（170vmax）、`::before` 背景 rgb(28,31,38)、椭圆遮罩 3549.6×2448**，末帧类清空。

### R34-3 三条探针教训（都进了注释）
1. **`screenshot` 会等渲染稳定**：t=40ms 请求的截图拍到的是 820ms 动画之后的收尾帧——用逐帧 `requestAnimationFrame` 采样，不用截图判时序。
2. **像素统计把顶栏黑字当成墨**：`<100` 阈值下顶栏「王磊 W.L.」也是暗像素，害我一度以为墨盖住了顶栏。测「墨在哪」要限定区域或按连通域。
3. **只差 hash 的 `goto` 走片段导航、脚本不重跑**——同一坑本轮重排测试时又踩一次，注释已写死顺序。

### R34-4 门与提交
build rc=0 · 预算门四行 PASS（滚动/手卷 3768 B）· 本厅 e2e **25/25** · 城侧转场 spec **3/3**。`53be9e9`。
`自看：楼侧 hold · shots/transit/hall-hold35-v11.png · 毛边成纤维状、落点与椭圆比对上；首帧 · 逐帧采样（截图不可用，见 R34-3）。`

## R35 · 移动端盲区自查：三处自造假绿（2026-09-04 14:1x–14:4x）

双评（agy C / seed D / agy W11）都只看桌面 1440×900。补测 375×667 与 reduced-motion，一次带出三处**属性绿、画面错**：

### R35-1 落点写死视口百分比 → 移动端偏 11vh
canvas 顶边随版心变：桌面 y=69/高 900 → 引擎 y=.60 落到视口 46.8%；移动 375×667 时 y=114/高 667 → **57.1%**。
我按桌面实测写死 `46.8vh`，移动端就偏了 11vh。
修：`[data-yin-drop]="0.36,0.40"` 挂在 canvas 上（与 `ink.drop(0.36,0.60)` 同源，引擎 y 原点在左下），
Arrive 读 `getBoundingClientRect()` 实时解算靶心；椭圆比例 = canvas 宽高比 × .925（桌面实测 1.6→1.48 定的系数）。
实测对齐：桌面 (36.3%,47.7%) vs S0 (36.1,46.8)；移动 (36.4%,56.1%) vs S0 (41.3,58.1)。

### R35-2 🔴 自定义属性兜底值写在自身，盖掉 JS 逐帧写的值
先写在伪元素规则里 → 再挪到元素自身 → **两次都盖掉从 html 继承的 JS 值**，遮罩恒为 170vmax（满屏墨）。
指纹很毒：`mask-image` 的**横向**半径已按 ratio 算出来了（看着像在动），纵向卡死在 170vmax；
而 e2e 读的是 `style` 属性、照样全绿。判据：CSS 自定义属性**只有元素 inline style 压得住自身类规则**，JS 要写就写那一面。
与 skill 规则 41（author display 盖 UA `[hidden]`）同型：都是「层叠优先级」造出的假绿。

### R35-3 🔴 filter 与 mask 又回到同一元素（v10 已实证过一次）
把首帧墨帘做成 `html.nx-transit::before` 时顺手把 mask+filter 都放了上去 → CSS 里 filter 先于 mask 作用，
位移的是**未被遮罩的纯色矩形**，等于没位移，边缘退回几何椭圆（移动端出图才看见，桌面因为墨块大没注意）。
修：head 只放**纯色**墨帘（0 计算、首绘即在，只负责顶住跨文档白闪期）；两层 DOM 接管后外层 filter、内层 mask+墨色。
附带：Arrive 必须排在 Yin 之后（要读 canvas，排前面 `querySelector` 返回 null，三个变量全空退兜底）。

### R35-4 门与提交
build rc=0 · 预算门四行 PASS（3768 B）· 本厅 e2e **25/25** · 城侧转场 **3/3**。`424ebef`。
residual：移动端毛刺相对偏大（displacement scale 46+13 是按 1440 宽调的），可按视口缩放，属打磨项。
`自看：移动端 hold · shots/transit/v16-mob-hold.png · 洇散边成立、靶心随画布下移；桌面 · v16-desk-hold.png · 同。`

## R36 · 全量 e2e 收口 + 双路审计（agy 二轮验修 / grok 会话审计）（2026-09-04 14:4x–15:4x）

### R36-1 合并后全量五项目 e2e：**137/137 passed（1.7h）**
`/tmp/nx-e2e-full-merged.log` 末行 `137 passed (1.7h)`，零失败零 flaky。跑的是 merge origin/main 之后重建的 dist（R33-4 已核现役）。
比 R31-1 的 131 多 6 条 = 本厅新增到达墨幕 2×2 + 城侧转场 spec 3 条 + 席位口径门。

### R36-2 agy 二轮验修（`out/W12-agy-verify.md`）：桌面已修、移动端三条新问题
| 条 | agy 判 | 亲核 | 处置 |
|---|---|---|---|
| P0-1 落点 | 桌面**已修**（它自测质心 35.94/47.66 vs S0 36.19/47.64，偏差 0.02vh）；移动端「比例 0.576 竖茄子」 | 桌面数字与我的实测互证（异源收敛）；移动端属实——比例绑 canvas 宽高比是我拍的，竖屏下失真 | 采纳，待改：比例锁 1.15~1.25 并按宽度缩放 |
| P1-a 毛边 | 桌面已修；移动端位移占屏宽 15.7% 成「獠牙」 | 属实 | 采纳，待改：≤768px 时 scale 12+4 |
| C-3 落款出画被切 | 「右侧竖排被裁、朱印丢失」 | **误报**：实测文字 right=351 < 视口 375，`document.scrollWidth == innerWidth == 375` 无横向溢出。它把 750×1334 的 2× 截图当 CSS 像素读了 | 驳回 |
| P0-2/P0-3/P2 | 「本轮图无法判」 | 判得对——我给的是楼侧 hold 帧，城侧墨拓/起笔/HUD 不在图上 | 记：下轮补城侧帧 |
| B 顶栏溢出 | 维持原判，要求顶栏隐藏 | 见 R36-3 | 部分采纳（口径型） |

### R36-3 grok 会话审计（`out/W13-grok-session-audit.json`，读 jsonl + 现役树，工具全开）
**逐条亲核后的裁决：**
| 条 | 亲核 | 处置 |
|---|---|---|
| **P0-2 公开 HTML 带 `rule:~/.claude/rules/`** | **属实且最严重**：产物实测命中 **18 次**，把作者机器目录结构印进公开页 | 已修 `9345bc4`：改相对 id `rule:<name>`，产物 18 → **0** |
| **P0-1 同页 3025 vs 600 无披露** | **属实**：`totals.sessions=3025`、明细 600；印阵有「此处陈列 24 / 80」，墨流没有对等披露 | 已修：可见小注「画面陈列 600 滴 / 全期 3025 次」+ 无障碍文本同款 |
| **P1-4 首帧断言骗得过** | **属实**：读 `::before` 的 computed `backgroundColor`，`display:none` 时照样返回墨色 | 已修：判据改「这一帧存在铺满视口的墨层」（两层其一）+ 末帧必须退场；**注入负控实测 rc=1** |
| P1-7 顶栏溢出应改属实 | **证否**：v16 桌面帧到达条纵带内暗像素占比 **3.4%**，未相交 | 维持驳回，但顶栏在转场期的观感属口径型 → 留待磊哥 |
| P0-3 sha256 64 位 + 精确 token | **属实存在**（`receipts[].sha256` 64 hex、`totals.tokens=113996514`、`days[].tokens` 精确） | **未修**，进 residual：涉及 reducer 口径与 R1 上抛结论，需先核 R1 原文再动 |
| P1-5 无 JS 海报未挂 noscript | 待核 | residual |
| P1-6 `identity_ok=false` 空集、白文印死代码 | 待核（ledger 实为 true 75 / null 5 / false 0） | residual |
| P1-8 `generatedFrom` 缺席仍绿 | 待核 | residual |
| P1-9 城侧 e2e 只锁 class 不锁画面 | 部分属实（RM 2×2 确实缺） | residual |
| P2-10~13 | 多条属实（引擎体积 INDEX 未更新、loader 恒 0、W3r 关闭口径） | residual |

**grok 的根因判断我接受**：门与 e2e 大量锁的是「实现自己写下的 DOM 属性」，而产品声称是「画面 / 隐私 / 绑定 / 全量数字」——R35 已在用这把尺子打自己，但首帧门与 P0 三条当时还停在属性层。
...
## R38 · 擦尾批次：跨楼缺陷 + 隐私收口 + about 厅评估（2026-09-04 17:3x–18:2x）

磊哥令「需要的，你把屁股擦完」——授权修第三条 + 补 about 视觉评估，按「擦尾」理解为把可自主处置的 residual 一并收掉。

### R38-1 跨楼旧到达卡藏掉身份条（grok P1-4）→ 已修 `74adc42`
`HallChrome.astro` 原 `if (parsed.poi && parsed.poi !== poi) return;` 会把楼名 + 探索 + 「返回科技城」一起藏掉。
改为：合法 query 即显示身份条，只丢别人楼的驾驶短句，并清掉陈旧卡。
**门自证**：新增 2×2；注入负控（把修复回滚成 `return`）实测 **rc=1**、报「身份条必须仍然可见」；还原后 rc=0。
共用组件改动 → about 厅 e2e 16/16 不回归。

### R38-2 隐私收口（grok P0-3）→ 已修 `f217f99`
两条台账**早已拍板但执行只做了一层**：
- R1 第 43 行「sha256 只存前 12 位，默认按建议做」→ 实现一直 64 位全值（且 reducer 注释自称「gate 要求 `^[0-9a-f]{64}$`」——**全仓没有任何门这样要求**，是未坐实自述被当前提）。
- R「精确 tokens 删除替换为量级桶」→ 只做了 `sessions[]`，`totals.tokens` 与 `days[].tokens` 两个口袋原样保留；而隐私门**只扫 sessions[]**，于是三处指纹在公开 JSON 里一直全绿。
处置：reducer 改源头 + 对在仓 ledger 定向转换（**不重新生成**）+ verify 门扩到 totals/days/receipts。
产物实测：精确 token 总量 **0 命中**、64 位全量 sha **0 命中**；数据完整性不变（3025/5/40/80）。
🔴 **重跑陷阱已写进 reducer 头部**：本机源数据已不完整，直接重跑产出 **1069 会话 / 1 席位 / 32 天 / 0 收据** 的残片，
且**所有门仍然绿**（门只查自洽，不查「是不是比上次少了一半」）。我实跑撞到了这个坑，已还原。

### R38-3 about 厅视觉评估：**第一轮作废，责任在我的派单素材**
首轮出图脚本按「幕顶对齐」取样，而该页揭示动画（`clip-path`）在**幕内 30% 才 reveal=1**——
拍到的全是动画起点（图被裁掉 73.5%）。agy 基于这批图判 5 分、4 个 P0，**逐条亲核 0/4 成立**：
| agy P0 | 亲核 | 裁决 |
|---|---|---|
| S1–S5「死黑开天窗、图未装配」 | 图**全部加载成功**（naturalWidth 1280、opacity 1），是父容器 clip-path 的动画中间态；实测 reveal 在幕内 30% 到 1 | **采样伪影** |
| S7 文案被固定地轨腰斩 | 滚到文档底部实测 `被地轨遮:false` | **滚动中间态** |
| 移动端六站字号 0.62rem≈10px、触控不达标 | 实测每项 **53×55px、字号 17px**、`scrollWidth==innerWidth` 无溢出 | **误报**（第二次把 2× 图当 CSS 像素读） |
| S1 顶部残留光丝 | 同属滚动中间态 | 未单独核，随批作废 |
**教训**：派单素材错会让审计员为它看到的现象编出**技术上说得通的归因**（它把 clip-path 机制说对了，却把动画中间态当成缺陷），
于是四条 P0 全部落空而每条读起来都成立。已按「幕内 45%」重出图，并在新派单里显式告知取样口径、fixed 栏语义、2× 缩放。

### R38-4 我自己看图记的两条（不依赖审计席）
- 驾驶态**三层提示同屏**（状态条 + 全键位条 + 「键位 H」按钮），同一件事说三遍占掉画面下部一条带。
- about 厅部分幕（如 s4）**上方约三分之一是空的**，内容重心偏下。
两条都属观感，不是功能缺陷，进 residual。

### R38-5 about 厅第二轮评估同样作废；换方法自评
第二轮已在派单里写明取样口径（幕内 45%）、fixed 栏语义、2× 缩放，agy 仍给 49/100、4 个 P0，
但四条依旧是「长幕的某个滚动位置」：`s1` 幕高 **2700px** vs 视口 900px，**任何单一滚动位置都只能拍到幕的一部分**。
→ 判据：**用静态截图审长滚动页，方法本身不成立**（审计席无法从一帧判断「这是不是终态」）。停止第三轮派单（有界停门）。

**换取样法自评**：实测每幕真正的内容块高 **824px < 视口 900px**——内容本就能完整装进一屏，
之前按 section 百分比取样必然拍到块与块之间的夹缝。改为「内容块滚到视口垂直居中」重出图（`shots/about3/`）：
- `08-s7.png`：六边形能力图 + 两张方向卡 + 脚注，进度轨在底部**不遮挡任何内容**
- `09-s8.png`：三张问题卡「看做法」链接**完整可见**、讲者简介、四出口 + 驾车回城，均无遮挡
agy 两轮共 8 条 P0 **全部由取样造成，0 条成立**。

**我的自评（about 厅）**：首屏强（3D 角色 + 光缆桥 + 六站进度轴），幕次干净、层级清楚、移动端合规
（53×55px 触控、17px 字号、无横向溢出）。**质量在线，约 80**，非 agy 的 49。
真正的打磨项（我看图记的）：部分幕内容重心偏下、卡片高度固定导致内容少时留白偏大、六边形图占位大而信息密度低。

### R38-6 水墨厅移动端两条（agy W12）
- **毛刺 scale 属实** → 按视口等比缩，移动端占屏 **15.7% → 4.8%**（桌面 4.1%），`654a72f`。
- **「墨团竖茄子应锁 1.15~1.25」驳回**：实测桌面 canvas 1.6 → 真墨团 **1.524**；移动 canvas 0.562 → 真墨团 **0.587**——
  那滴墨在竖屏下本来就是竖椭圆（引擎按归一化坐标渲染）。遮罩要贴合的是它，锁固定比例反而交棒时形状穿帮。
  🔴 但根因记 residual：**墨形随屏幕比例变形是引擎坐标映射问题**（该按短边等比），改动波及所有幕，需整套重做视觉验收。

### R38-7 本批次门与提交
`74adc42`（跨楼身份条 + 2×2 + 负控 rc=1）· `f217f99`（隐私收口）· `654a72f`（毛刺缩放）· `f68b3c8`（e2e 上界按实测）。
每次提交前：build · 预算门 · check-links · about-hall-gate · ledger-verify · ledger-gate 全 rc=0；
nexus 厅 e2e **26/26**、about 厅 e2e **16/16**（共用组件改动不回归）。

### R38-8 仍未做的 residual（交磊哥拍板或后续批次）
| 项 | 类别 | 说明 |
|---|---|---|
| 回城续驶（grok P1-1） | **口径型** | 回城 `?poi=` 走 wandering、丢 V/环视/键位卡、任务线清零。要不要「续驶协议」是产品决策 |
| 375 回城再拦（grok P1-3） | **口径型** | 从楼里回城仍是首访封面「进入科技城」。免门还是改文案，需拍 |
| 驾驶三层提示同屏 | 观感 | 状态条 + 全键位条 + 「键位 H」说同一件事三遍 |
| 引擎坐标映射致墨形拉伸 | 工程 | 影响所有幕，需整套重做视觉验收 |

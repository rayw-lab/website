...
软件渲染 10fps 下每秒墙钟只推进 0.33 秒 —— 600 滴的构图在 24 秒里只落了约 120 滴。
**画面塌成左下一团，而引擎不报错、ready 正常、海报照常生成。**
更值得记的是：同一文件里 `idleSince` 那条注释写的就是同一个错（"空转判据必须用真实墙钟"），
当时**只修了空转判据，没修叙事时钟**。判据：
...
初始化耗掉的两秒，在第一帧一次性要求补落约 85 滴 ≈ 2000 次绘制，渲染线程当场堵死。
症状是**构建期截图 30s 超时拿不到稳定帧**，而页面本身 200、无报错、canvas 在位。
修法：每帧补齐上限 12 滴（60fps 只需 0.7 滴/帧，快设备永不触顶；慢设备只是构图变长不会卡死）。

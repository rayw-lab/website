...
> **版本**：v1.1（2026-08-24，B0 一揽子修订，依据 `docs/spec/audit-report-v1.1.md` §9 P0-1/P0-5：§2.2 Work 更新频率统一为每季度；§2.3 URL 结构补 `/lab/`、`/world/`、`/en/` 三页、`/404`；§4.1 确立为案例 12 模块唯一事实源并吸收 PRD 增量；第 6 章动效行增加两条在册豁免（首页 Hero 实时层 + 循环动画配额；`/world/` 沉浸展项）；§8.1 英文范围对齐 PRD ABT-06/WORK-10；§12.1 增加与 PRD §10 指标体系的映射。其余章节不变）
...
| **Work** | 能力证明主阵地，案例库 | 三旗舰案例；按支柱/行业/角色筛选的索引 | 每季度新增或深度迭代 1 篇（v1.1 与 §11.3、PRD §8 对齐） | 联系我聊类似问题 |
...
/ai-lab/{slug}/         # 单篇实验记录
/lab/{slug}/            # 重型交互 Demo 独立路由（Lab 子系统，与 /ai-lab/ 实验记录分离）
/world/                 # 智能座舱试验场（Hybrid 旗舰 Lab；opt-in 独立路由，不进六大导航）
/about/                 # 关于
/now/                   # Now 页面（About 子页，独立 URL 便于外链）
/contact/               # 联系
/en/  /en/about/  /en/contact/   # 英文三页（双向 hreflang，见 8.1）
...
（v1.1 修订注，审计 P0-5/M-10 消解：`/lab/{slug}`、`/world/`、`/en/` 三页与 `/404` 为 v1.1 补录，口径与 PRD §5.2、SRD §12.7.1、鸟瞰图 §3.2 一致。）
...
**canonical 声明与增量吸收（v1.1 修订注，审计 P0-1 裁决）**：

- 本表是全站「案例 12 模块」的**唯一事实源（canonical）**——PRD（WORK-02/WORK-07）、SRD（§8.1 work schema `modules` 字段）与一切下游文档一律引用本表编号，不得复制或另立清单；
- 精简版最小集 = 模块 **1/2/6/9/10**（背景与语境 / 问题定义 / 解决方案 / 结果与量化 / 证据）——含结果与证据、语义自洽的最小对外案例；
- PRD v1.1 曾并行存在的另一版清单已废弃，其两项真增量按如下方式吸收，均**不占用模块编号**：①「一句话摘要」由案例详情页顶部的 30 秒结论区承载（PRD WORK-04）；②「信息边界」作为每篇案例必备的发布件「信息边界声明」附于正文末尾（PRD GOV-05）。
...
| 动效 | 仅保留必要的 hover 与页面渐入；无滚动劫持、无视差（豁免见下方注记） |
| 密度 | 首页信息密度高但层级清晰；详情页阅读体验优先 |

**动效豁免注记（v1.1 修订，承接 SRD R4 行动项 / adaptation §10.3 M1 / 审计 P0-5，即鸟瞰图 B0）**——以下两条豁免自 v1.1 起生效，本表其余条款（无滚动劫持、无视差）在全部路由继续有效：

1. **首页 Hero 同源实时渲染层豁免**：Hero 实车舞台（PRD HOME-07）允许复用配置器资产做 WebGPU 车模慢速自转等实时渲染动效；全站同屏循环动画配额 ≤ 2 处（SRD NFR-P5）；`prefers-reduced-motion`/移动端/无 WebGL 永远停留静态 poster，性能门禁与降级触发条件按 PRD HOME-07/08 执行；
2. **`/world/` 独立路由沉浸式展项豁免**：opt-in 独立路由的沉浸式 3D 展项（PRD LAB-16，Hybrid 决策）不受本表「动效仅 hover 与渐入」约束，改受 SRD §12.7.2 独立运行时预算约束（世界内同屏循环动画 ≤ 5 处，与首页配额分账）；首页与内容页对世界零字节依赖（SRD AP-9）。
...
| 国际化 | 站点以中文为主；V1 提供首页/About/Contact 三页英文版并加双向 `hreflang`（PRD ABT-06）；Work 案例英文摘要属 V2 范围（PRD WORK-10）——v1.1 与 PRD/SRD 对齐（审计 m-9 消解） |
...
**与 PRD §10 指标体系的映射（v1.1 修订注，审计 P0-5）**：PRD §10 以本章四大作用为顶层框架，细化为「北极星 + 7 指标」体系（含 v1.1 世界子指标与数据阀门）。两者按下表对应；指标的执行定义与验收口径以 PRD §10 为准，本章保留业务视角的顶层框架与复盘机制（12.2 节）：

| 本章四大作用 | PRD §10 对应指标 |
|------|------|
| 职业定位 | 指标 1（10 秒定位达成率）、指标 5（品牌词自然搜索表现） |
| 能力证明 | 指标 2（首页→证据点击率）、指标 3（旗舰案例阅读深度）、指标 4（Demo 参与度，含世界子指标） |
| 知识影响力 | 指标 5（长尾词自然搜索）、指标 6（内容资产引用与 RSS） |
| 关系转化 | 北极星（月度高质量 inbound ≥ 2）、指标 7（回访与材料带走） |
